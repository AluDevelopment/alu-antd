...
describe('Notification.placement', () => {
  ...
  it('change notification placement by `open` method', () => {
    ...
    style = getStyle($$('.alu-notification-topLeft')[0]);
    ...
    expect($$('.alu-notification-topLeft').length).toBe(1);

    // topRight
    open({
      placement: 'topRight',
    });
    style = getStyle($$('.alu-notification-topRight')[0]);
    expect(style.top).toBe(defaultTop);
    ...
    expect($$('.alu-notification-topRight').length).toBe(1);

    // bottomRight
    open({
      placement: 'bottomRight',
    });
    style = getStyle($$('.alu-notification-bottomRight')[0]);
    ...
    expect($$('.alu-notification-bottomRight').length).toBe(1);

    // bottomLeft
    open({
      placement: 'bottomLeft',
    });
    style = getStyle($$('.alu-notification-bottomLeft')[0]);
    expect(style.top).toBe('');
    expect(style.left).toBe('0px');
    expect(style.bottom).toBe(defaultBottom);
    ...
    expect($$('.alu-notification-bottomLeft').length).toBe(1);
  });
  ...
  it('change notification mountNode by `config` method', () => {
    ...
    expect($container.querySelector('.alu-notification')).not.toBe(null);
    $container.remove();
  });
});
